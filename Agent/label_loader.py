...
class LabelLoader:
    ...
    # `dumpsys package` is a fixed ~3-5s cost, so it needs to earn its keep.
    DUMPSYS_MIN_UNCOVERED = 5
    ...
    def extract_label_for_package(self, pkg: str) -> Optional[str]:
        ...
        try:
            ...
            # Pull APK to temp (always fresh, use 120s timeout for large APKs)
            if os.path.exists(local_apk):
                os.remove(local_apk)
            self.adb.run(["pull", apk_path, local_apk], timeout=120)
    ...
    def auto_extract_all_labels(self, packages: Set[str]) -> Dict[str, str]:
        """
        Extract labels for ALL given packages using FAST strategy:
        1. KNOWN_LABELS (instant) — Samsung/Google system apps
        2. Batch dumpsys (fast) — no APK pulls needed
        3. aapt2 (slow fallback) — only for remaining apps
        
        This is 100x faster than pulling all APKs sequentially.
        """
        if not self.adb:
            print("⚠️ ADB not available — cannot extract labels")
            return {}

        total = len(packages)
        labels = {}

        print(f"🔧 First launch: extracting labels for {total} apps...")
        print(f"   Strategy: KNOWN → dumpsys → aapt2\n")

        # STEP 1: KNOWN_LABELS (instant, 60-80% of apps covered)
        known_count = 0
        for pkg in packages:
            if pkg in KNOWN_LABELS:
                labels[pkg] = KNOWN_LABELS[pkg]
                known_count += 1

        if known_count > 0:
            print(f"   ✅ KNOWN_LABELS: {known_count} found")

        # STEP 2: Batch dumpsys (fast, no APK pulls)
        remaining = packages - set(labels.keys())
        if remaining and self.adb:
            try:
                dumpsys_labels = self.batch_fetch_from_dumpsys(remaining)
                labels.update(dumpsys_labels)
                if dumpsys_labels:
                    print(f"   ✅ Dumpsys: {len(dumpsys_labels)} found")
            except Exception as e:
                print(f"   ⚠️ Dumpsys failed: {e}")

        # STEP 3: aapt2 only for remaining (slow but necessary for some apps)
        remaining = packages - set(labels.keys())
        if remaining and self.aapt2_path:
            print(f"   🔧 aapt2 for {len(remaining)} remaining apps (may take a minute)...")
            failed = 0

            for i, pkg in enumerate(sorted(remaining), 1):
                if i % 10 == 0 or i == len(remaining):
                    print(f"      [{i}/{len(remaining)}] Extracting... ({len(labels)} total found)")
            ...
            if failed > 0:
                print(f"   ⚠️ aapt2: {len(remaining) - failed} found, {failed} failed/skipped")
            else:
                print(f"   ✅ aapt2: {len(remaining)} found")

        # STEP 4: Fallback for anything still missing
        for pkg in packages:
            if pkg not in labels:
                labels[pkg] = self.label_from_package_name(pkg)
        ...
        print(f"\n   ✅ Extraction complete!")
        print(f"      Total labels: {len(labels)}/{total}")
        print(f"      No label: {total - len(labels)}")
    ...
    def initialize(self, current_packages: Set[str]) -> Dict[str, str]:
        ...
        if is_first_launch:
            ...
            # A full `dumpsys package` costs ~3-5s regardless of how many labels
            # we actually need, so only pay it when enough apps are genuinely
            # unknown. For a handful, a derived name is good enough and the
            # install listener refines it later.
            dumpsys_labels = {}
            unknown = {p for p in uncovered if p not in KNOWN_LABELS}
            if len(unknown) >= self.DUMPSYS_MIN_UNCOVERED:
                dumpsys_labels = self.batch_fetch_from_dumpsys(uncovered)
            elif unknown:
                print(f"⏭️ Skipped dumpsys for {len(unknown)} unlabeled app(s)")
...
class PackageInstallListener:
    ...
    def _handle_install(self, pkg: str) -> None:
        ...
        # Runs on the poll thread — go through CONSOLE so it can't land in the
        # middle of a command the user is typing.
        CONSOLE.notice(f"📲 New app detected: {pkg}")
        ...
        if label:
            ...
            label = fallback
            CONSOLE.notice(f"   ⚠️ Could not extract label, using: {fallback}")
    ...
    def _handle_uninstall(self, pkg: str) -> None:
        ...
        self.label_loader.save_disk_cache()
        CONSOLE.notice(f"🗑️ App removed: {pkg} ({label})")

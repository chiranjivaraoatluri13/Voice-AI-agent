...
SYSTEM_FALLBACKS = {
    ...
    "google search": ["com.google.android.googlequicksearchbox", "com.android.chrome"],
...
class AppResolver:
    ...
    def _installed(self, pkg: str) -> bool:
        """Check once per session; `pm path` is an ADB round trip on the hot path."""
        cached = self._installed_cache.get(pkg)
        if cached is not None:
            return cached
        if pkg in self.label_cache:
            self._installed_cache[pkg] = True
            return True
        try:
            ...
            result = False
        self._installed_cache[pkg] = result
        return result
